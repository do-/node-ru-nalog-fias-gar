...
## `getVersion`
```js
const version = await garXmlZip.getVersion () // {version: 'v.238', date: '2026-03-10'}
```
Асинхронный метод разбора файла `version.txt`, который должен располагаться в корне архива.

В официальной [документации](https://fias.nalog.ru/docs/Описание%20службы%20получения%20обновлений.docx) формат данных не описан, лишь упомянут _"файл version.txt, в котором хранится дата, за которую была сделана актуальная выгрузка"_.

Разбор реализован, исходя из следующего содержимого:
