...
const GarXmlZip = class  {
	...
	async getVersion () {
		...
		if (!('version.txt' in dir)) throw new Error ('version.txt not found')

		const stream = await zip.open (this.path, 'version.txt')

		const chunks = []

		for await (const chunk of stream) chunks.push (chunk)

		const [dateLine, versionLine] = Buffer.concat (chunks).toString ().split ('\n')

		return {version: versionLine.trim (), date: dateLine.trim ().replace (/\./g, '-')}
