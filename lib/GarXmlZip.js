...
const GarXmlZip = class  {
	...
	async getVersion () {
		...
		const dir = await zip.list (this.path), FILE_NAME = 'version.txt'

		if (!(FILE_NAME in dir)) throw Error (`${FILE_NAME} not found`)

		{

			const {size} = dir [FILE_NAME]

			if (size < 10)   throw Error (`${FILE_NAME} is too small: ${size} bytes`)

			if (size > 4096) throw Error (`${FILE_NAME} is too large: ${size} bytes`)

		}		 

		const text = await zip.read (this.path, FILE_NAME)

		const [dateLine, versionLine] = text.split ('\n')

		return {version: versionLine.trim (), date: dateLine.trim ().replaceAll ('.', '-')}
